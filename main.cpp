...
int main()
{
	...
	while (true)
	{
		...
		const auto& localPlayer = memory.Read<std::intptr_t>(client + offset::dwLocalPlayer);
		const auto& shotsFired = memory.Read<std::int32_t>(localPlayer + offset::m_iShotsFired);

		if (shotsFired)
		{
			const auto& clientState = memory.Read<std::intptr_t>(engine + offset::dwClientState);
